...
class TALLDetector(DeepfakeDetector):

    def __init__(self, model_name: str = 'TALL', config: str = 'tall.yaml', device: str = 'cpu'):
    ...
    def build_backbone(self):
        model_kwargs = dict(
            ...
            ape=self.config['ape'],
            thumbnail_rows=self.config['thumbnail_rows'], 
            drop_rate=self.config['drop_rate'],
            drop_path_rate=self.config['drop_path_rate'], 
            use_checkpoint=False, 
            bottleneck=False,
            duration=self.config['clip_size'],
            device=self.device)

        default_cfg = {
            'url': self.config['pretrained'],
            'num_classes': 1000, 
        ...
        backbone = SwinTransformer(img_size=self.config['resolution'], **model_kwargs)
    ...
    def load_model(self):
        ...
        checkpoint = torch.load(Path(WEIGHTS_DIR) / self.weights, map_location=self.device)
        self.model.load_state_dict(checkpoint, strict=True)        

    def __call__(self, data_dict):        
        with torch.no_grad():
            return self.forward(data_dict, inference=True)['prob']

    def build_loss(self):
        # prepare the loss function
        loss_class = LOSSFUNC[self.config['loss_func']]
    ...
    def forward(self, data_dict: dict, inference=False) -> dict:
        ...
        if inference:
            pred_dict = {k: t.detach() for k, t in pred_dict.items()}
        return pred_dict
