    ...
    try:
        ...
        for i, sample in enumerate(dataset):
            ...
            try:
                ...
                # Expect 3-class output: [real, synthetic, semisynthetic]
                if probabilities.shape[-1] != 3:
                    if verbosity >= 1:
                        bt.logging.warning(f"  Image {i+1} unexpected output shape {probabilities.shape}, skipping sample")
                    continue
                ...
                # Predicted class index in 3-class output
                predicted_index = int(np.argmax(pred_probs))
                
                # Determine true label: 2 if inpainting models, else 1 (synthetic)
                true_label = 2 if isinstance(model_name, str) and ("inpainting" in model_name.lower()) else 1
                ...
                if verbosity >= 1:
                    ...
                    if max_samples is not None:
                        bt.logging.info(f"{result_symbol} Image {i+1}/{max_samples}: {'CORRECT' if is_correct else 'WRONG'} (Label={true_label}, Pred={predicted_index}, Model={model_name})")
                    else:
                        bt.logging.info(f"{result_symbol} Image {i+1}: {'CORRECT' if is_correct else 'WRONG'} (Label={true_label}, Pred={predicted_index}, Model={model_name})")
        ...
        with video_payload(temp_dir=temp_dir, dataset_config=dataset_config) as temp_dir:
            ...
            for i, sample in enumerate(dataset):
                ...
                try:
                    ...
                    if probabilities.shape[-1] != 3:
                        if verbosity >= 1:
                            bt.logging.warning(f"  Video {i+1} unexpected output shape {probabilities.shape}, skipping sample")
                        continue
                    pred_probs = probabilities[0]
                    ...
                    if verbosity >= 1:
                        result_symbol = "✅" if is_correct else "❌"
                        if max_samples is not None:
                            bt.logging.info(f"{result_symbol} Video {i+1}/{max_samples}: {'CORRECT' if is_correct else 'WRONG'} (True={true_label}, Pred={predicted_index}, Frames={len(frames)}, Model={model_name})")
                        else:
                            bt.logging.info(f"{result_symbol} Video {i+1}: {'CORRECT' if is_correct else 'WRONG'} (True={true_label}, Pred={predicted_index}, Frames={len(frames)}, Model={model_name})")
                    total += 1
