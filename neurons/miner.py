...
class Miner(BaseNeuron):
    ...
    async def detect_video(self, request: Request):
        ...
        if content_type not in ("video/avi", "application/octet-stream"):
            bt.logging.warning(
                f"Unexpected content type: {content_type}, expected video/avi"
            )
        try:
            ...
            with av.open(temp_path) as container:
                ...
                try:
                    ...
                    bt.logging.warning(f"Could not get codec info: {str(codec_err)}")
