...
class GenerativeMiner(BaseNeuron):
    ...
    def init(self):
        ...
        if not self.external_ip:
            self. external_ip = requests.get(
                "https://checkip.amazonaws.com", timeout=10).text.strip()
    ...
    async def _process_task(self, task):
        ...
        bt.logging.info(f"🎯 Processing task {task.task_id}: {task.task_type}/{task.modality}")
        ...
        try:
            ...
            service = self.service_registry.get_service(task.task_type, task.modality)
            if not service:
                raise ValueError(f"No service available for {task.task_type}/{task.modality}")
            ...
            self.task_manager.mark_task_completed(task.task_id, result.get("data"), result.get("url"))
            send_success_webhook(
                ...
                retry_delay=self.config.miner.webhook_retry_delay,
                timeout=self.config.miner.webhook_timeout)
            bt.logging.success(f"✅ Task {task.task_id} completed in {processing_time:.2f}s")
    ...
    async def _call_service_process(self, service, task):
        ...
        if inspect.iscoroutinefunction(service.process):
            # Async service - await the result
            return await service.process(task)
        else:
            # Sync service - call directly
            return service.process(task)
    
    def _log_available_services(self):
        """Log available services for each task type after initialization."""
        bt.logging.info("🔍 Checking service availability for all task types...")
        
        # Define all task types and modalities
        task_types = [
            ("image_generation", "image"),
            ("video_generation", "video"), 
            ("image_modification", "image"),
            ("video_modification", "video")
        ]
        
        available_count = 0
        total_endpoints = len(task_types)
        
        bt.logging.info("=" * 60)
        bt.logging.info("📋 GENERATIVE MINER SERVICE STATUS")
        bt.logging.info("=" * 60)
        
        for task_type, modality in task_types:
            service = self.service_registry.get_service(task_type, modality)
            
            # Map task types to endpoint names for cleaner display
            endpoint_map = {
                ("image_generation", "image"): "POST /gen_image",
                ("video_generation", "video"): "POST /gen_video",
                ("image_modification", "image"): "POST /mod_image", 
                ("video_modification", "video"): "POST /mod_video"
            }
            
            endpoint = endpoint_map.get((task_type, modality), f"{task_type}/{modality}")
        ...
        if available_count == 0:
            ...
            bt.logging.warning(f"⚠️  Partial functionality: {total_endpoints - available_count} endpoints unavailable")
        else:
    ...
    async def generate_image(self, request: Request):
        ...
        return await self._handle_generation_request(request, "image_generation", "image")
    
    async def generate_video(self, request: Request):
        """Submit video generation task."""
        return await self._handle_generation_request(request, "video_generation", "video")
    
    async def modify_image(self, request: Request):
        """Submit image modification task."""
        return await self._handle_modification_request(request, "image_modification", "image")
    
    async def modify_video(self, request: Request):
        """Submit video modification task."""
        return await self._handle_modification_request(request, "video_modification", "video")
    
    async def _handle_generation_request(self, request: Request, task_type: str, modality: str):
        """Generic handler for generation requests."""
        try:
            ...
            if not webhook_url:
                return self._error_response("Missing X-Webhook-URL header", 400)
            ...
            if len(prompt) > max_prompt_length:
                return self._error_response(f"Prompt too long (max {max_prompt_length} chars)", 400)
            ...
            if modality == "image":
                ...
                if width > max_width or height > max_height:
                    return self._error_response(f"Image dimensions too large (max {max_width}x{max_height})", 400)
            ...
            return self._success_response({
                ...
                "estimated_time": self._estimate_processing_time(task_type, modality)
            }, 202)
            
        except Exception as e:
            bt.logging.error(f"Error handling generation request: {e}")
            return self._error_response(str(e), 500)
    
    async def _handle_modification_request(self, request: Request, task_type: str, modality: str):
        """Generic handler for modification requests."""
        try:
            input_data = await request.body()
            webhook_url = request.headers.get("X-Webhook-URL")
            prompt = request.headers.get("X-Modification-Prompt", "")
            signed_by = request.headers.get("Epistula-Signed-By", "")
            ...
            if not prompt:
                return self._error_response("Missing X-Modification-Prompt header", 400)
            
            if not input_data:
                return self._error_response("Missing input data", 400)
            
            # Validate input file size
            max_size_mb = 50
            input_size_mb = len(input_data) / (1024 * 1024)
            if input_size_mb > max_size_mb:
                return self._error_response(f"Input file too large (max {max_size_mb}MB)", 400)
            ...
            bt.logging.error(f"Error handling modification request: {e}")
            return self._error_response(str(e), 500)
    
    def _estimate_processing_time(self, task_type: str, modality: str) -> int:
        """Estimate processing time in seconds based on task type."""
        estimates = {
            ("image_generation", "image"): 30,
            ("image_modification", "image"): 45,
            ("video_generation", "video"): 300,
            ("video_modification", "video"): 360,
        }
        return estimates.get((task_type, modality), 60)
    ...
    async def get_miner_info(self, request: Request):
        ...
        return self._success_response({
            ...
            "config": {
                "max_concurrent_tasks": getattr(self.config.miner, 'max_concurrent_tasks', 5),
                "task_timeout": getattr(self.config.miner, 'task_timeout', 300),
                "max_task_age_hours": getattr(self.config.miner, 'max_task_age_hours', 24),
                "cleanup_interval": getattr(self.config.miner, 'cleanup_interval', 3600),
            },
    ...
    def _serve_on_network(self) -> bool:
        ...
        try:            
            bt.logging.info(f"🌐 Serving axon on {self.external_ip}:{self.config.axon.port}")
