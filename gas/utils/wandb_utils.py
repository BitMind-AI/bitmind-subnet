...
class WandbLogger:
    ...
    def _log_challenge_results(self, results, media_sample, media_uuids, aug_params=None):
        ...
        Args:
            results: List of dictionaries containing miner response results
            media_uuids: List of UUIDs of the associated media
            aug_params: Dictionary containing augmentation parameters
        """
        run = self._ensure_run()
        ...
        # Extract rewards and scores if available
        if successful_results:
            rewards = [r.get("reward", 0) for r in successful_results]
            scores = [r.get("score", 0) for r in successful_results]
            
            log_data["summary"]["avg_reward"] = sum(rewards) / len(rewards) if rewards else 0
            log_data["summary"]["avg_score"] = sum(scores) / len(scores) if scores else 0
            log_data["summary"]["min_reward"] = min(rewards) if rewards else 0
            log_data["summary"]["max_reward"] = max(rewards) if rewards else 0

        run.log(log_data)
        bt.logging.info(f"Logged challenge results with media UUIDs: {media_uuids}")

    def log(self, challenge_results, media_sample, aug_params):
        """
