...
class ContentDB:
    ...
    def _init_database(self):
        ...
        with self._get_db_connection() as conn:
            ...
            # Add model_name column to generator_challenge_outcomes (migration)
            try:
                conn.execute("ALTER TABLE generator_challenge_outcomes ADD COLUMN model_name TEXT")
                bt.logging.info("Added 'model_name' column to generator_challenge_outcomes table")
                conn.commit()
            except Exception as e:
                if "duplicate column name" not in str(e).lower() and "already exists" not in str(e).lower():
                    bt.logging.error(f"Unexpected error adding 'model_name' column: {e}")
    ...
    def mark_challenge_outcome_for_media(
        ...
        When status is 'verified', also copies the model_name from the
        media table into the outcome row so reward computation can read
        per-modality model_names directly from the outcomes table.
        """
        try:
            with self._get_db_connection() as conn:
                if status == "verified":
                    # Pull model_name from media and store on outcome in one pass
                    cursor = conn.execute(
                        """
                        UPDATE generator_challenge_outcomes
                        SET status = ?,
                            ...
                            media_id = COALESCE(media_id, ?),
                            model_name = COALESCE(
                                (SELECT model_name FROM media WHERE id = ?),
                                model_name
                            ),
                            updated_at = ?
                        WHERE media_id = ?
    ...
    def get_challenge_outcome_stats_last_n_hours(
        ...
        for hotkey, stats in miner_stats.items():
            ...
            img_v = stats["image_verified"]
            vid_v = stats["video_verified"]
            img_f = stats["image_failed"]
            vid_f = stats["video_failed"]
            img_total = img_v + img_f
            vid_total = vid_v + vid_f
            verification_stats[hotkey] = {
                ...
                "image_failed": img_f,
                "image_pass_rate": (img_v / img_total) if img_total > 0 else 0.0,
                "image_model_names": stats.get("image_model_names", []),
                "video_verified": vid_v,
                "video_failed": vid_f,
                "video_pass_rate": (vid_v / vid_total) if vid_total > 0 else 0.0,
