...
class ContentDB:
    ...
    def _init_database(self):
        ...
        with self._get_db_connection() as conn:
            ...
            # Create generator_liveness table for tracking when generators last submitted
            # This persists across validator restarts for reliable liveness filtering
            conn.execute(
                """
                CREATE TABLE IF NOT EXISTS generator_liveness (
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_generator_liveness_last_seen ON generator_liveness (last_seen)"
            )
    ...
    # -------------------------------------------------------------------------
    # Generator Liveness Tracking
    # -------------------------------------------------------------------------

    def update_generator_liveness(self, hotkey: str, last_seen: float, uid: int = None) -> bool:
        """
        Update or insert generator liveness record.
        
        Args:
            hotkey: Generator's SS58 address
            last_seen: Unix timestamp of last activity
            uid: Optional UID of the generator
        ...
        try:
            with self._get_db_connection() as conn:
                conn.execute(
                    """
                    INSERT INTO generator_liveness (hotkey, last_seen, uid)
                    VALUES (?, ?, ?)
                    ON CONFLICT(hotkey) DO UPDATE SET 
                        last_seen = excluded.last_seen,
                        uid = COALESCE(excluded.uid, generator_liveness.uid)
                    """,
                    (hotkey, last_seen, uid),
                )
                conn.commit()
                return True
        except Exception as e:
            bt.logging.error(f"Error updating generator liveness: {e}")
            return False

    def get_all_generator_liveness(self, max_age_hours: int = 168) -> dict:
        """
        Get all generator liveness records within the age limit.
        
        Args:
            max_age_hours: Maximum age in hours to include (default 168 = 7 days)
            
        Returns:
            Dict mapping hotkey to last_seen timestamp
        """
        try:
            with self._get_db_connection() as conn:
                ...
                cursor = conn.execute(
                    """
                    SELECT hotkey, last_seen FROM generator_liveness
                    WHERE last_seen >= ?
                    """,
                    (cutoff_time,),
                )
            ...
            bt.logging.error(f"Error getting generator liveness: {e}")
            return {}

    def prune_old_generator_liveness(self, max_age_hours: int = 168) -> int:
        """
        Remove generator liveness records older than max_age_hours.
        ...
        try:
            with self._get_db_connection() as conn:
                ...
                cursor = conn.execute(
                    "DELETE FROM generator_liveness WHERE last_seen < ?",
                    (cutoff_time,),
                )
                conn.commit()
                ...
                if pruned > 0:
                    bt.logging.debug(f"Pruned {pruned} old generator liveness records")
                return pruned
        except Exception as e:
            bt.logging.error(f"Error pruning generator liveness: {e}")
            return 0
