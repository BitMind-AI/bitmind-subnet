...
def calculate_splits(total_items, num_splits):
    ...
    return [(i * chunk_size, (i + 1) * chunk_size - 1) if i < num_splits - 1 else (i * chunk_size, total_items - 1) for i in range(num_splits)]

def load_and_combine_datasets(hf_org, dataset_name, model, splits):
    dataset_parts = []
    ...
    combined_dataset = concatenate_datasets(dataset_parts)
...
def parse_arguments():
    ...
    -- "bitmind" "celeb-a-hq" "stable-diffusion-xl-base-1.0" 30000 4 \
        "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"
    """
...
def main():
    ...
    combined_dataset_name = f"{args.hf_org}___{args.model_name}"
    
    # Calculate splits based on total items and number of splits
    splits = calculate_splits(args.total_items, args.num_splits)
    
    combined_dataset = load_and_combine_datasets(args.hf_org, args.dataset_name, args.model_name, splits)
